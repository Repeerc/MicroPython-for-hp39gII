...
volatile unsigned char address_page_data[5];

volatile unsigned char __DMA_NAND_AUXILIARY_BUFFER[65]  __attribute__((aligned(0x200)));
//接口时序，单位ns
...
volatile unsigned char commandAddressBuffer[10] __attribute__((aligned(0x200)));
unsigned char id[10];

struct nand_flash_dev foundChip;

int is_gpmi_dma_operation_completed()
{
    return dmaOperationCompleted;
}

int is_gpmi_ecc_operation_completed()
{
    return eccOperationCompleted;
}
...
void ecc8_completion_irq_handle() {
	...
	/*	uartdbg_printf("ECC8:finish. Error Level:%x %x %x %x\n",
