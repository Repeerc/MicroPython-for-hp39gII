...
extern unsigned int VRAM_ADDR;

uint8_t *screen_buffer = (uint8_t*) &VRAM_ADDR; //显存
...
void LCD_write_dat(unsigned int dat, unsigned int dat_size) {
    ...
    screen_parameter_dma_desc.p_DMABuffer = ((uint8_t *)&dat);

    while (HW_APBH_CHn_SEMA(0).B.INCREMENT_SEMA)
        ;
    BF_WRn(APBH_CHn_NXTCMDAR, 0, CMD_ADDR, (reg32_t)((uint8_t *)&screen_parameter_dma_desc));
...
void LCD_write_cmd(unsigned int cmd, unsigned int cmd_size) {
    ...
    screen_command_dma_desc.p_DMABuffer = ((uint8_t *)&cmd);

    while (HW_APBH_CHn_SEMA(0).B.INCREMENT_SEMA)
        ;
    BF_WRn(APBH_CHn_NXTCMDAR, 0, CMD_ADDR, (reg32_t)((uint8_t *)&screen_command_dma_desc));
    BW_APBH_CHn_SEMA_INCREMENT_SEMA(0, 1);
}
...
void init_chains_flush_frame() {
    ...
    //============================== CMD 1 =====================================
    chains_flush_frame[0].p_Next = (struct _hw_lcdif_DmaDesc *)((uint8_t *)&chains_flush_frame[1]);
    ...
    chains_flush_frame[0].p_DMABuffer = ((uint8_t *)"\x2A");
    ...
    chains_flush_frame[1].p_Next = (struct _hw_lcdif_DmaDesc *)((uint8_t *)&chains_flush_frame[2]);
    ...
    chains_flush_frame[1].p_DMABuffer = ((uint8_t *)"\x00\x00\x00\x55");
    ...
    chains_flush_frame[2].p_Next = (struct _hw_lcdif_DmaDesc *)((uint8_t *)&chains_flush_frame[3]);
    ...
    chains_flush_frame[2].p_DMABuffer = ((uint8_t *)"\x2B");
    ...
    chains_flush_frame[3].p_Next = (struct _hw_lcdif_DmaDesc *)((uint8_t *)&chains_flush_frame[4]);
    ...
    chains_flush_frame[3].p_DMABuffer = ((uint8_t *)"\x00\x00\x00\x87");
    ...
    chains_flush_frame[4].p_Next = (struct _hw_lcdif_DmaDesc *)((uint8_t *)&chains_flush_frame[5]);
    ...
    chains_flush_frame[4].p_DMABuffer = ((uint8_t *)"\x2C");
    ...
    chains_flush_frame[5].p_DMABuffer = ((uint8_t *)screen_buffer);
...
void LCD_dma_channel_reset(void) {
    ...
    //设置LCD DMA通道命令描述符
    screen_buffer_dma_desc[0].p_Next = (struct _hw_lcdif_DmaDesc *)((uint8_t *)&screen_buffer_dma_desc[1]);
    ...
    screen_buffer_dma_desc[0].p_DMABuffer = ((uint8_t *)"\x2c");
    ...
    screen_buffer_dma_desc[1].p_DMABuffer = ((uint8_t *)screen_buffer);
...
*/
    ...
    BF_WRn(APBH_CHn_NXTCMDAR, 0, CMD_ADDR, (reg32_t)((uint8_t *)&chains_flush_frame[0]));
    BW_APBH_CHn_SEMA_INCREMENT_SEMA(0, 1);
}
